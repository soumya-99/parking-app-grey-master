...
const CarReportsFixed = ({navigation}) => {
  ...
  const handleUnbilledPrint = async () => {
    ...
    let payload =
      '--------------------------------\n';
    payload += `DT: ${date.toLocaleDateString(
      'en-GB',
    )} TM: ${date.toLocaleTimeString(undefined, options)}\n`;
    payload += `FROM: ${mydateFrom.toLocaleDateString(
      'en-GB',
    ...
    payload +=
      '--------------------------------\n';
    // payload += `TOTAL ${''.padEnd(5)}    ${totalQTY}${''
    //   .toString()
    //   .padEnd(9)} ${totalAdvance
    //   .toString()
    //   .padEnd(13)} ${totalPrice.toString()} \n  `;
  ...
  function abc(vehicleType, quantity, TotalAdvance, totalAmount) {
    ...
    const paddedVehicleType = vehicleType
      .toString()
      .substring(0, vehicleTypeLength);
    const paddedQuantity = quantity.toString().padStart(quantityLength, ' ');
    const paddedTotalAdvance = TotalAdvance.toString().padStart(
      TotalAdvanceLength,
      ' ',
    );
  ...
  async function handleGenerateReport() {
    ...
    setLoading(true);
    getVehicleWiseFixedReports(mydateFrom.toISOString(), mydateTo.toISOString())
      .then(res => {
        console.log(
          '------------------Vehicle wise reports data Error----------------',
          res,
        );
        setUnbilledData(res);
              ...
              <View style={styles.container}>
                ...
                <View
                  ...
                  <Text style={[styles.cell, styles.hcell]}>
                    {unbilledData && totalQTY}
                  </Text>
                  <Text style={[styles.cell, styles.hcell]}>
                    {unbilledData && totalAdvance}
                  </Text>
                  <Text style={[styles.cell, styles.hcell]}>
                    {unbilledData && totalPrice}
                  </Text>
                  {/* <Text style={[styles.cell]}>{item.age}</Text> */}
                </View>
                <View style={{}}>
