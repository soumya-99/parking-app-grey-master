...
const OperatorReportFixed = ({navigation}) => {
  ...
  const {getOperatorWiseFixedReports} = VehicleInOutStore();
  ...
  async function handleGenerateReport() {
    ...
    getOperatorWiseFixedReports(
      mydateFrom.toISOString(),
      mydateTo.toISOString(),
    )
      .then(res => {
      ...
      .catch(error => {
        setLoading(false);
        console.error(
          '------------------operator wise reports data Error----------------',
          error,
        );
      });
  }
  ...
  return (
    <View style={{flex: 1}}>
      <CustomHeader
        title={'Oprator Wise Fixed Receipts'}
...
export default OperatorReportFixed;
