...
function VehicleInOutStore() {
  ...
  const calculateStatistics = async (devMod) => {
    const db = await getDatabaseConnection();
    ...
    return new Promise((resolve, reject) => {
      db.transaction(tx => {
        tx.executeSql(
          `SELECT
             (SELECT COUNT(*) FROM vehicleInOutTable 
             WHERE date_time_in >= ? AND date_time_in <= ?) AS vehicleInCount,
             (SELECT COUNT(*) FROM vehicleInOutTable 
             WHERE (date_time_Out >= ? AND date_time_out <= ?) AND date_time_out IS NOT NULL) AS vehicleOutCount,

            ${(devMod=='F')?'COALESCE((SELECT SUM(paid_amt) FROM vehicleInOutTable WHERE date_time_in >= ? AND date_time_in <= ?), 0) AS totalAmount':'COALESCE((SELECT SUM(paid_amt) FROM vehicleInOutTable WHERE date_time_out >= ? AND date_time_out <= ?), 0) AS totalAmount'}
             `
